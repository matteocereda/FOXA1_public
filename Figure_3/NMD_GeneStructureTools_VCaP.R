...
whippet_file_directory <- "sources/Whippet_raw_files/VCaP/"
...
tmp = subset(tmp,key%in%subset(delta,Type=='CE' & abs(DeltaPsi)>0.01 & !Complexity=='K0')$key)
wds2@diffSplicingResults = tmp
tmp = wds2@readCounts
tmp$key = paste0(tmp$Gene,'_',tmp$Node,'_',tmp$Type)
tmp = subset(tmp,key%in%subset(delta,Type=='CE' & abs(DeltaPsi)>0.01 & !Complexity=='K0')$key)
wds2@readCounts = tmp
wds.ce <- filterWhippetEvents(wds2, eventTypes = 'CE', psiDelta=0.01,probability=0)
...
save(orfs_normal,file='Rdata/NMD_analysis_GeneStructureTools_SENSITIVE_VCaP_complem.Rdata')
message('[*] orfs_skipped')
orfs_skipped <- getOrfs(skippedExonTranscripts[skippedExonTranscripts$set == 
                                                 "skipped_exon"],
                        BSgenome = g,
                        returnLongestOnly = FALSE, allFrames = TRUE, uORFs=TRUE)
save(orfs_normal,orfs_skipped,file='Rdata/NMD_analysis_GeneStructureTools_SENSITIVE_VCaP.Rdata')
message('[*] orfs_included')
orfs_included <- getOrfs(skippedExonTranscripts[skippedExonTranscripts$set == 
                                                  "included_exon"],
                         BSgenome = g,
                         returnLongestOnly = FALSE, allFrames = TRUE, uORFs=TRUE)
save(orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_SENSITIVE_VCaP.Rdata')
...
orfChange <- orfDiff(orfsX = orfs_included, 
                     ...
                     compareUTR=TRUE,
                     allORFs = orfs_normal)
save(orfChange,orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_SENSITIVE_VCaP.Rdata')
message('[*] nmdChange')
...
save(orfChange,nmdChange,orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_SENSITIVE_VCaP.Rdata')

## COMPLEMENTARY TO SENSITIVE ===========
library(notNMD)
wds2 = wds
tmp = wds2@diffSplicingResults
tmp$key = paste0(tmp$gene,'_',tmp$node,'_',tmp$type)
...
orfs_normal <- getOrfs(normalTranscripts, BSgenome = g, 
                       returnLongestOnly = FALSE, allFrames = TRUE, uORFs=TRUE)
save(orfs_normal,file='Rdata/NMD_analysis_GeneStructureTools_COMPLEMENTARY_TO_SENSITIVE_VCaP_complem.Rdata')
message('[*] orfs_skipped')
orfs_skipped <- getOrfs(skippedExonTranscripts[skippedExonTranscripts$set == 
                                                 "skipped_exon"],
                        BSgenome = g,
                        returnLongestOnly = FALSE, allFrames = TRUE, uORFs=TRUE)
save(orfs_normal,orfs_skipped,file='Rdata/NMD_analysis_GeneStructureTools_COMPLEMENTARY_TO_SENSITIVE_VCaP.Rdata')
message('[*] orfs_included')
orfs_included <- getOrfs(skippedExonTranscripts[skippedExonTranscripts$set == 
                                                  "included_exon"],
                         BSgenome = g,
                         returnLongestOnly = FALSE, allFrames = TRUE, uORFs=TRUE)
save(orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_COMPLEMENTARY_TO_SENSITIVE_VCaP.Rdata')
...
orfChange <- orfDiff(orfsX = orfs_included, 
                     ...
                     compareUTR=TRUE,
                     allORFs = orfs_normal)
save(orfChange,orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_COMPLEMENTARY_TO_SENSITIVE_VCaP.Rdata')
message('[*] nmdChange')
nmdChange <- attrChangeAltSpliced(orfs_included,orfs_skipped,
                                  attribute="nmd_prob",
                                  compareBy="gene",
...
save(orfChange,nmdChange,orfs_normal,orfs_skipped,orfs_included,file='Rdata/NMD_analysis_GeneStructureTools_COMPLEMENTARY_TO_SENSITIVE_VCaP.Rdata')