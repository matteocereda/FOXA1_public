source("./config_FOXA1_analysis_2022_05.R")

pr_folder = "FOXA1_reg_vs_const"
...
PATH_PR_FOLDER     = p(PATH_RNAMOTIFS, "/m3_light/results/", pr_folder,"/")
...
d1            = readRDS(PATH_ALL_DATASET)
d1$delta.mean = d1$delta.mean/100
d1            = d1 %>% subset(overall.mean>=0.999 & abs(delta.mean)<2.5e-5)

# Each row must be formatted as:
# row_id; second_id; chrom; strand; up_exon_end; exon_start; exon_end; dw_exon_start; diRank

cntr <- with(d1, data.frame(event_id  = event_id %>% cha %>% strsplit("_") %>% sapply(tail,1) %>% num,
...
d2            = d2 %>% subset(FOXA1_sign)

reg  = with(d2, data.frame(event_id  = event_id %>% cha %>% strsplit("_") %>% sapply(tail,1) %>% num,
                           event_chr = event_chr,
                           strand    = strand %>% cha,
                           coord     = event_coordinates %>% cha %>%  strsplit(":") %>% sapply(function(y){num(y[2:5])}) %>% t,
...
setwd(PATH_RNAMOTIFS)
system(p("./RNAmotifs.sh ", PATH_RNAMOTIFS, " ", pr_folder, " ", pr_folder, "_input_RNAmotifs.txt hg19 15 0.5 200 10000 4 0.1 0.0005 1000 1000 > logs/", pr_folder, ".log"))


# Compute PWMs ----

system(p("python ./extended_motifs.py ",pr_folder))
...
enr_tets = read.table(p(PATH_RES_RNAMOTIFS,"enriched_tetramers.txt"), stringsAsFactors = F)$V1
motifs   = c()
...
clusterExport(clus,list("pr_folder", "ncol_PWMs", "RNAmotifs", "motifs","p", "PATH_RES", "PATH_PR_FOLDER","PATH_PWM_FOLDER", "PATH_TO_DB"))

setwd(PATH_TO_DB)
PWMs <- grep("HepG2",list.files(pattern=".txt$"), value = T)

setwd(PATH_TO_MACROAPE)
...
write.csv(res, p(PATH_TO_MACROAPE,"/",pr_folder,"_",ncol_PWMs,"_columns.csv"), row.names = F)
...
p_cutoff =  p_enh %>% rename(c("r1enh_pFis" = "r1sil_pFis",
  ...
  rename(c("r1sil_pFis" = "r1_pFis_0.01",
...
l1 = try({list_type_barplot1 = do.call(list,lapply(enr_tets, function(x){get_exons_with_tetramer_region1(x,PATH_RES_RNAMOTIFS, PATH_INP_RNAMOTIFS)}))}, silent=T)
...
colfun3   = colorRamp2(breaks = c((exp(seq(lq[2],lq[3],length.out=20))-m)/s,
                                  (exp(seq(lq[3],lq[4],length.out=50))-m)/s),
